...
class VisualSearchGenerator:
    ...
    def _generate_background(self, grain_intensity=35):
        # 1. Generate haphazard light background noise
        w_base = max(10, self.width // 40)
        h_base = max(10, self.height // 40)
        ...
        light_bg = Image.blend(img1, img2, alpha=0.4)
        
        # 2. Generate organic dark patches
        dark_layer = Image.new('L', (self.width, self.height), 255)
        draw = ImageDraw.Draw(dark_layer)
        
        cx = self.width // 2
        cy = self.height // 2
        blur_radius = 40
        
        # Inner margin prevents blobs from bleeding across quadrant centerlines after the blur
        margin = int(blur_radius * 1.5) 
        
        # Defined as (inner_max_x, inner_min_x, inner_max_y, inner_min_y)
        # 'None' implies an outer boundary where it's free to touch or go off-screen
        quad_configs = [
            (cx - margin, None, cy - margin, None),       # Top-Left
            (None, cx + margin, cy - margin, None),       # Top-Right
            (cx - margin, None, None, cy + margin),       # Bottom-Left
            (None, cx + margin, None, cy + margin)        # Bottom-Right
        ]
        
        dark_centers_hires = []
        
        for q_idx, (imax_x, imin_x, imax_y, imin_y) in enumerate(quad_configs):
            # Pick a safe starting center for the organic blob
            min_sx = imin_x if imin_x else 50
            max_sx = imax_x if imax_x else self.width - 50
            min_sy = imin_y if imin_y else 50
            max_sy = imax_y if imax_y else self.height - 50
            
            # Ensure max bounds are valid for small images
            max_sx = max(min_sx, max_sx)
            max_sy = max(min_sy, max_sy)
            
            start_x = random.randint(min_sx, max_sx)
            start_y = random.randint(min_sy, max_sy)
            dark_centers_hires.append((start_x, start_y))
            
            curr_x, curr_y = start_x, start_y
            num_nodes = random.randint(7, 15) # Random walk steps for complexity
            
            for _ in range(num_nodes):
                r = random.randint(30, 70)
                
                # Strictly clamp the circles so they do not cross inner margins
                if imax_x and (curr_x + r > imax_x): curr_x = imax_x - r
                if imin_x and (curr_x - r < imin_x): curr_x = imin_x + r
                if imax_y and (curr_y + r > imax_y): curr_y = imax_y - r
                if imin_y and (curr_y - r < imin_y): curr_y = imin_y + r
                
                # Draw the node
                shade = random.randint(20, 60)
                draw.ellipse([curr_x - r, curr_y - r, curr_x + r, curr_y + r], fill=shade)
                
                # Wander for next node
                curr_x += random.randint(-50, 50)
                curr_y += random.randint(-50, 50)
                
        # 3. Combine and apply effects
        # 'darker' safely imprints the dark blobs without ruining the light noise
        smooth_bg = ImageChops.darker(light_bg, dark_layer)
        smooth_bg = smooth_bg.filter(ImageFilter.GaussianBlur(radius=blur_radius))
        ...
        return final_bg, smooth_bg, dark_centers_hires
    ...
    def generate_stimulus(self, 
                          total_TL, num_L, total_QO, num_O, 
                          size_small, size_large, stroke_width, min_distance, target_on_patch_prob,
                          opts_O, opts_Q, opts_T, opts_L, dark_threshold=120,
                          rotations=[0, 90, 180, 270]):
        
        img, smooth_bg, dark_centers = self._generate_background()
        ...
        targets = [i for i in items_to_draw if i[3]]
        distractors = [i for i in items_to_draw if not i[3]]
        
        random.shuffle(targets)
        random.shuffle(distractors)
        
        guaranteed_items = []
        # Assign 4 items to strictly map to the 4 dark quadrants, prioritizing targets
        while len(guaranteed_items) < 4 and (targets or distractors):
            if targets:
                guaranteed_items.append(targets.pop(0))
            else:
                guaranteed_items.append(distractors.pop(0))
                
        remaining_items = targets + distractors
        random.shuffle(remaining_items)
        items_to_place = guaranteed_items + remaining_items

        locations = []
        max_attempts = 1500
        dark_patch_index = 0
        
        for shape, color, size, is_target in items_to_place:
            placed = False
            attempts = 0
            
            is_guaranteed_patch = dark_patch_index < len(dark_centers)
            needs_dark_patch = is_target and (random.random() < target_on_patch_prob)

            while not placed and attempts < max_attempts:
                if is_guaranteed_patch:
                    # Force near the dark patch center (with slight jitter inside the blob)
                    cx, cy = dark_centers[dark_patch_index]
                    x = cx + random.randint(-30, 30)
                    y = cy + random.randint(-30, 30)
                    
                    x = max(min_distance, min(x, self.width - min_distance))
                    y = max(min_distance, min(y, self.height - min_distance))
                else:
                ...
                if not is_guaranteed_patch and needs_dark_patch:
        ...
        metadata = []
        for (shape, color, size, _), (x, y) in zip(items_to_place, locations):
            ...
            metadata.append({
                ...
                "bg_dark": bool(bg_luminance <= dark_threshold) 
            })
...
target_on_patch_prob = st.sidebar.slider("Additional Target on Patch Prob.", 0.0, 1.0, 0.85,
                                         help="Beyond the 4 mandatory spots, what is the chance remaining targets seek dark areas?")
dark_threshold = st.sidebar.slider("Dark Patch Threshold (Luminance)", 50, 220, 120, 
                                   help="Lower values mean non-mandatory targets will only spawn in the darkest spots.")

st.sidebar.header("4. Item Counts")
total_TL = st.sidebar.slider("Total T's and L's", 0, 100, 40)
num_L = st.sidebar.slider("Number of L's (Targets)", 0, total_TL, min(10, total_TL))

total_QO = st.sidebar.slider("Total Q's and O's", 0, 100, 40)
num_O = st.sidebar.slider("Number of O's (Targets)", 0, total_QO, min(10, total_QO))
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            for i in range(num_images):
                img, metadata = generator.generate_stimulus(
                    total_TL=total_TL, num_L=num_L, total_QO=total_QO, num_O=num_O,
